...
import { createPhieuDichVu, getAllDichVu, getAllNhomDichVu } from 'src/service/DichVu'
import GhiChuModal from '../modal/GhiChuModal'
import CurrencyInput from 'react-currency-input-field'
const HangHoaModal = ({ visible, onClose, onSubmit }) => {
  ...
  // Lọc danh sách dịch vụ theo nhóm đang chọn
  ...
  const handleSubmit = () => {
    if (selectedProducts.length === 0) {
      addToast(exampleToast('⚠️ Vui lòng chọn ít nhất một dịch vụ'))
      return
    }

    // Gửi danh sách dịch vụ đã chọn về component cha
  ...
  return (
    <>
          ...
          <div className="flex flex-col ">
            ...
            <div className="flex gap-3">
              ...
              <div className="w-3/6 border-r border-dashed pr-3">
                ...
                {/* Danh sách sản phẩm */}
                <div
                  className="grid grid-cols-4 gap-4 p-4 overflow-y-auto h-96"
      ...
      <GhiChuModal visible={visibleGhiChu} onClose={() => setVisibleGhiChu(false)} />
    </>
  )
}

HangHoaModal.propTypes = {
