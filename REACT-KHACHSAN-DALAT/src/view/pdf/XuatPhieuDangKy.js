              ...
              <View style={styles.column7}>
                <View>
                  ...
                  <View style={[styles.formRow, { marginTop: -5 }]}>
                    <Text style={styles.formLabel}>
                      Tên đơn vị/Company:{' '}
                      {thongTinKhachHang?.ten_nhom_khach_hang === 'Walk-in'
                        ? 'Walk - in'
                        : thongTinKhachHang?.loai_nguon_khach}
                    </Text>
                  </View>
                  {/* <View style={styles.formRow}>
                  <Text style={styles.formLabel}>
                ...
                <View style={[styles.table, { marginBottom: '2' }]}>
                    ...
                    thongTinThanhToan.map((row, index) => (
                      <View key={index} style={styles.tableRow}>
                        ...
                        <View style={[styles.emptyCell, { width: '17%' }]}>
                          <Text>{formatCurrency(row.don_gia)}</Text>
                        </View>
                  ...
                  <View style={styles.totalRow}>
                    ...
                    >
                      <Text style={styles.totalText}>Tổng phụ thu/</Text>
                      <Text style={styles.totalItalicText}>Total surcharge</Text>
                    </View>
                    ...
                    >
                      <Text>{formatCurrency(tongPhuThuTreEm + tongTienExtra)}</Text>
                    </View>
                  ...
                  <View style={styles.totalRow}>
                    ...
                    >
                      <Text>{formatCurrency(thongTinKhachHang?.tien_coc)}</Text>
                    </View>
...
const App = () => {
  ...
  const originalPreviewRef = useRef(null) // Lưu ảnh gốc để mỗi lần ký đều composite từ ảnh sạch
  const handleDownloadImage = useCallback(async () => {
    ...
    try {
      ...
      // Nếu chỉ có 1 trang thì render trực tiếp
      if (pdfDoc.numPages === 1) {
        ...
        // Gom nhiều trang thành 1 ảnh duy nhất (nối dọc)
        let totalHeight = 0
        let maxWidth = 0

        // Tính tổng chiều cao và chiều rộng lớn nhất
        ...
        const link = document.createElement('a')
        link.download = `PhieuDangKy_${thongTinKhachHang.ma_booking}_all.png`
    ...
    if (isMobile && thongTinKhachHang && thongTinThanhToan) {
      const generatePreview = async () => {
        ...
        try {
          ...
          const arrayBuffer = await blob.arrayBuffer()
          const pdfDoc = await pdfjsLib.getDocument({ data: arrayBuffer }).promise
          const scale = 2 // Scale nhỏ hơn chút để render nhanh hơn trên mobile
  ...
  const handleSignatureConfirm = useCallback(
    (signatureDataUrl) => {
      ...
      previewImg.onload = () => {
        ...
        // Luôn vẽ từ ảnh gốc → xóa chữ ký cũ tự động
        ctx.drawImage(previewImg, 0, 0)

        // Tọa độ vùng ký (column5 = 30% bên phải, vùng dashed box)
        const sigBoxX = w * 0.715
        const sigBoxY = h * 0.60
        const sigBoxW = w * 0.270
        const sigBoxH = h * 0.45

        // Vẽ chữ ký lên, thu nhỏ để vừa khung, căn giữa
