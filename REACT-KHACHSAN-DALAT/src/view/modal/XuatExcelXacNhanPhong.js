...
import { exportExcelChiTietDatPhong } from 'src/service/ThongKeXuatExcelService'

const XuatExcelXacNhanPhong = ({ visible, onClose, ma_booking }) => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      setLoadExcel(true)
      const blob = await exportExcelChiTietDatPhong(ma_booking)
      ...
      const fileName = `XacNhanDatPhong_${yyyy}${MM}${dd}${HH}${mm}${ss}.xlsx`
  ...
  return (
    <>
      ...
      >
        ...
        <CModalBody>
          <div>
            <span>Bạn có muốn xuất file excel xác nhận phòng cho booking này? </span>
          </div>
        </CModalBody>
        <CModalFooter>
...
XuatExcelXacNhanPhong.propTypes = {
...
export default XuatExcelXacNhanPhong
