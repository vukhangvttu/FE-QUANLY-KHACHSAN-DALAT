...
import { deleteKhachHangPhong } from 'src/service/KhacHangPhongService'

const XoaThongTinKhach_Phong = ({ visible, onClose, ma_khachhang_phong, onSubmit }) => {
  const navigate = useNavigate()

  const [trangthaiload, setTrangthaiload] = useState(false)

  const onClickDeleteXepPhong = async (ma_khachhang_phong) => {
    if (ma_khachhang_phong === null || ma_khachhang_phong === undefined) {
      return addToast(exampleToast('⚠️ Mã booking hiện không hợp lệ'))
    } else {
      try {
        setTrangthaiload(true)
        // 5. Gọi API nếu dữ liệu hợp lệ
        const response = await deleteKhachHangPhong(ma_khachhang_phong, navigate)
  ...
  return (
    <>
      ...
      >
        ...
        <CModalBody>
          <h4>
            Bạn có muốn <span className="text-red-500">xóa thông tin khách hàng trong phòng</span>{' '}
            không, với <span>Mã khách: {ma_khachhang_phong}</span>
          </h4>
        </CModalBody>
        <CModalFooter>
          ...
          {!trangthaiload && (
            <CButton
              ...
              onClick={() => onClickDeleteXepPhong(ma_khachhang_phong)}
            >
