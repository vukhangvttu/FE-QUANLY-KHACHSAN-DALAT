...
import { deleteChiTietBooKing } from 'src/service/APIService'

const XoaLoaiPhongChiTiet = ({
  visible,
  onClose,
  ma_chitiet,
  ma_booking,
  ...
  const onClickDeleteLoaiPhong = async (ma_chitiet) => {
    if (ma_chitiet === null || ma_chitiet === undefined) {
      return addToast(exampleToast('⚠️ Mã booking hiện không hợp lệ'))
    } else {
      try {
        setTrangthaiload(true)
        // 5. Gọi API nếu dữ liệu hợp lệ
        const response = await deleteChiTietBooKing(ma_chitiet, ma_booking, navigate)
  ...
  return (
    <>
      ...
      >
        ...
        <CModalBody>
          <h4>
            Bạn có muốn <span className="text-red-500">xóa loại phong {tenloaiphong}</span> không?
          </h4>
        </CModalBody>
        <CModalFooter>
          ...
          {!trangthaiload && (
            <CButton
              ...
              onClick={() => onClickDeleteLoaiPhong(ma_chitiet)}
            >
