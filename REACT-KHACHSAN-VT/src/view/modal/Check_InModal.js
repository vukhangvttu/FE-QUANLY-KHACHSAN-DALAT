...
const Check_InModal = ({ visible, onClose, ma_xepphong_bookking, tenphong, onSubmit }) => {
    ...
    if (ma_xepphong_bookking === null || ma_xepphong_bookking === undefined) {
      ...
      try {
        ...
        if (response.code === 200) {
          if (response.result) {
            let successMessage = '✔️ Check-In nhận phòng thành công ' + tenphong
  ...
  return (
    <>
      ...
      >
        ...
        <CModalBody>
          <h4 className="mb-3">
            Bạn có muốn <span className="text-red-500">check-in</span> nhận phòng {tenphong} không ?
          </h4>
