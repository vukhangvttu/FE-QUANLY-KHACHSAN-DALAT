...
import { deleteXepPhongBooking } from 'src/service/XepPhongBooKingService'
import { useNavigate } from 'react-router-dom'

const XoaXepPhongModal = ({ visible, onClose, ma_booking, onSubmit }) => {
  const navigate = useNavigate()

  const [trangthaiload, setTrangthaiload] = useState(false)

  const onClickDeleteXepPhong = async (ma_booking) => {
    if (ma_booking === null || ma_booking === undefined) {
      ...
      try {
        ...
        const response = await deleteXepPhongBooking(ma_booking, navigate)

        console.log('xóa successfully:', response)
  ...
  return (
    <>
      ...
      >
        ...
        <CModalBody>
          <h4>
            Bạn có muốn <span className="text-red-500">xóa tất cả</span> thông tin xếp phòng không
          </h4>
        </CModalBody>
        <CModalFooter>
          ...
          {!trangthaiload && (
            <CButton
              ...
              onClick={() => onClickDeleteXepPhong(ma_booking)}
            >
...
export default XoaXepPhongModal
