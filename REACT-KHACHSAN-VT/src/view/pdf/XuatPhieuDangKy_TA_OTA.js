  ...
  const fetchData = async () => {
    try {
      ...
      const [khachHangData, thanhToanData, phuThuData] = await Promise.all([
        ...
        getThongTinXuatPhieuDangKyOTA_TA(ma_booking),
  ...
  const handleDownloadImage = useCallback(async () => {
    ...
    try {
      ...
      if (pdfDoc.numPages === 1) {
        ...
        link.download = `PhieuDangKy_TA_OTA_${thongTinKhachHang.ma_booking}.png`
        ...
        const link = document.createElement('a')
        link.download = `PhieuDangKy_TA_OTA_${thongTinKhachHang.ma_booking}_all.png`
